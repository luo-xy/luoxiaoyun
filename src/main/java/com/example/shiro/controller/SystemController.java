...
public class SystemController {
    ...
    public String updUser(String userName, ModelMap model) {
        User user = userService.findUserByName(userName);
        List<Role> selectRole=userService.findRoleByName(userName);
        List<Role> roles = roleService.getAllRoles();
        model.put("user",user);
        model.put("selectRole",selectRole);
        model.put("roles",roles);
        for (int i = 0; i < selectRole.size() ; i++) {
            System.out.println(selectRole.get(i));
        }
    ...
    public String updUser(User user,int [] id,ModelMap modelMap){
        int i = userService.updateUser(user, id);
        if(i>0){
            modelMap.put("msg","添加成功");
            return "success";
        }else{
            modelMap.put("msg","更新出错");
            return "error";
        }
